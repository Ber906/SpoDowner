...
def search_spotify():
    ...
    try:
        from utils.spotify import spotify_api
        tracks = spotify_api.search_tracks(query, limit)
        
        if tracks is None:
            # Add more detailed error message for debugging
            logger.error(f"Spotify search returned None for query: {query}")
            return jsonify({
                "error": "Failed to search Spotify",
                "message": "Search failed. The API may be experiencing issues."
            }), 500
        
        # Return in the expected format for the frontend
        return jsonify({"tracks": tracks})
    except Exception as e:
        logger.error(f"Error in search_spotify endpoint: {str(e)}")
        return jsonify({
            "error": "Search failed", 
            "message": f"Error: Search failed. {str(e)}"
        }), 500

@app.route('/api/track/<track_id>', methods=['GET'])
def get_spotify_track(track_id):
    """Get a specific track by ID from Spotify API"""
    from utils.spotify import spotify_api
...
@app.route('/')
def index():
    # Pass spotify_enabled flag to the template
    return render_template(
        'index.html', 
        user=current_user, 
        spotify_enabled=app.config.get('SPOTIFY_ENABLED', False)
    )
...
def start_download():
    try:
        ...
        track_id = request.json.get("track_id")  # New parameter for track ID
        
        # At least one of url or track_id is required
        if not url and not track_id:
            return jsonify({"error": "URL or track ID is required"}), 400

        # If track_id is provided, convert it to a Spotify URL
        if track_id:
            url = f"https://open.spotify.com/track/{track_id}"
        # Otherwise validate the URL
        elif not url.startswith("https://open.spotify.com/"):
        ...
        track_name = request.json.get("track_name", "Pending...")
        download = current_user.add_download(spotify_url=url, track_name=track_name)
        ...
        download_states[session_id] = {
            ...
            "track_name": track_name  # Store the track name for reference
        }
...
def cancel_download():
    ...
    # Debug logging
    app.logger.info(f"Cancel request received: Method={request.method}, Session ID={session_id}")
    
    if not session_id or session_id not in download_states:
        app.logger.warning(f"Cancel failed: Session ID not found in active downloads")
        return jsonify({"error": "No such session"}), 404
    ...
    try:
        ...
        if state.get("process"):
            try:
                ...
                app.logger.info(f"Process terminated for session {session_id}")
            except Exception as e:
                app.logger.warning(f"Could not terminate process: {str(e)}")
        else:
            app.logger.info(f"No process found for session {session_id}")
        
        # Try to remove the download folder if it exists
        folder_path = state.get("folder", "")
        if folder_path and os.path.exists(folder_path):
            try:
                shutil.rmtree(folder_path)
                app.logger.info(f"Removed folder {folder_path}")
            except Exception as e:
                app.logger.warning(f"Could not remove folder: {str(e)}")
        else:
            app.logger.info(f"No folder to remove for session {session_id}")
        
        # Always remove from download_states
        del download_states[session_id]
        app.logger.info(f"Session {session_id} removed from download_states")
